...
const validateName = value => {
    ...
    // } else if (!/^[A-ZÁÉÍÓÚÑ][a-záéíóúñü]{2,19}$/.test(value)) {
    } else {
        // const nameRegExp = new RegExp('^[A-ZÁÉÍÓÚÑ][a-záéíóúñü]{2,19}$');
        const nameRegExp = new RegExp(`^[A-ZÁÉÍÓÚÑ][a-záéíóúñü]{${minLength - 1},${maxLength - 1}}$`);
        if (!nameRegExp.test(value)) {
            message = `
            El nombre debe escribirse con la primera letra en mayúsculas y el
            resto en minúsculas. Solo se toman como válidos caracteres del español.
            `;
        }
    }
